...
- Preserves legacy CLI behavior: CLI-owned workflows do not gain API artifact
  ...
  exceptions.
- Adds synchronized pre-run construction for parallel NEB and AutoNEB so local
  calculator, engine, optimizer, and trajectory-writer failures exit all ranks
  before optimizer collectives.
...
- `python scripts/verify_wheel_api.py --mpi-smoke` with a bounded two-rank
  optimizer-construction failure regression.
