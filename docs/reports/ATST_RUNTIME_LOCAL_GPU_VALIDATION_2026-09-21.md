...
冷/热启动复核（同一 case，66 原子 relax/8 步/threads 4）：全新目录 12.76 s、
同目录重复 12.37 s / 13.70 s——进程级缓存对总墙钟无显著影响（±5%），与 §9 的
"固定成本在模型加载与首次预热"一致；P5 的"冷/热启动"记录可按同法直接复用。

采样开/关观测开销（同一清单、slots=1、各 3 次重复）：关闭 per-case 采样
27.41 / 26.65 / 27.16 s（中位 27.16），开启（`--case-telemetry`，1 s 间隔
sidecar 采样）26.85 / 26.45 / 26.90 s（中位 26.85）——差异 ≤0.4 s（<1.5%，
且方向为"开更略快"），**本机与 2 个 case 的规模下采样开销不可测**；P5 的
"采样开/关检查观测开销"可直接复用该 A/B（并应在大批/多卡下重测）。

## 15. NEB 图数 × rank 数本地压力测试（单卡；非 P5 结论）

用 FT²DP 科学体系（χ-Fe₅C₂ 118 原子，端点取自 `mission_20260920` 的
`TS1_IS.xyz`/`TS1_FS.xyz`）线性插值构造 4 内部图（`chain6`）与 8 内部图
（`chain10`）band，在同一张 RTX 2070 SUPER 上做串行 vs 多 rank 对照
（`mpi4py` 图像并行、`world_size == interior_images`、`max_steps` 3/2、
`endpoint_singlepoint: always`）：

| 变体 | 内部图 | ranks | 墙钟 | attempt | rank0 `dp.force_calls` | Σforce_calls | 显存峰值 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| serial_c6 | 4 | 1 | 16.97 s | 14.54 s | 46 | 46 | 4975 MiB |
| par4_c6 | 4 | 4 | 69.20 s | 62.84 s | 6 | 18 | 7630 MiB |
| serial_c10 | 8 | 1 | 20.37 s | 17.79 s | 66 | 66 | — |
| par8_c10 | 8 | 8 | 136.83 s | 124.98 s | 5 | 23 | 7710 MiB |

结论（本地边界）：单卡上按图分 rank **严重负收益**（4 rank ≈ 4.1×、8 rank ≈
7.7× 串行墙钟），显存峰值逼近 8 GiB 上限；并行侧实际只做了 18/26 次力调用
（串行 46/66 次），说明劣化来自同卡多 CUDA 上下文下**每次调用的延迟**，而不是
计算量。P5 矩阵的"10 ranks / 1 卡"一栏因此应按**压力边界**记录，不应期待收益；
"图数 ≤ 卡数"的收益假设仍必须在 V100 多卡现场验证（站点上下文/MPS 行为与
本机 WSL2 不同）。
