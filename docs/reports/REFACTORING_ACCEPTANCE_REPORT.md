...
| `examples/05_sella_H2-Au` | Sella | `394343` | `COMPLETED`, exit `0:0` | `sella_run/OUT.ABACUS/running_scf.log` generated |
| `examples/06_relax_H2-Au` | Relax | `394344` | `COMPLETED`, exit `0:0` | `relax_run/OUT.ABACUS/running_scf.log` generated |
| `examples/07_vibration_H2-Au` | Vibration | `394371` | `COMPLETED`, exit `0:0` | Previous job `394345` failed on stale zero-byte ASE vibration cache; workflow now cleans cache by default |
| `examples/08_d2s_Cy-Pt` | D2S | `394370` | `COMPLETED`, exit `0:0` | Previous job `394346` failed because ASE DyNEB reads endpoint energies; D2S now attaches endpoint calculators |

The completed SAI jobs verify the end-to-end `ASE -> ATST-Tools CLI/YAML ->
abacuslite -> ABACUS v3.10.1` path on SAI.
