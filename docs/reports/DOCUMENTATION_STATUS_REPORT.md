...
| `docs/superpowers/plans/2026-09-20-atst-gpu-node-tuning-plan.md` | plan | 执行中（P0–P5 已完成，P6 进行中）：先 atst 后平台的分期计划（P0–P6）；P0–P5 已交付并以本地 + SAI V100 证据闭合（P5 压力/收益矩阵见 SAI 报告 §5h/§5j），P6 待执行。未闭合项（TF 后端维度与 DP 容器对照为 `condition-blocked`；只读模型共享/重启回归、MPS 归属、站点 `mpiexec` 重绑定确认、P6 交付与平台接入为 `open`）见计划头部「剩余项（2026-09-22）」。 |
