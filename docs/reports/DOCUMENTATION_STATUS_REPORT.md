...
- 2026-09-22（**MPI 重绑定重复实测收口**）：站点连续 3 次通过 `mpiexec --oversubscribe -n 8` + `round_robin`（作业 1451595/96/97，节点 n13/n23/n26，wall 14.5/15.7/19.0 s，`devices.bound=true`、`counters_mpi.world_size=8` Σ`dp.force_calls=26`），旧挂起现象不可复现；切片 `docs/reports/data/ATST_MPI_REBIND_REPEAT_20260922/`。过程收获（写进切片）：作业必须用 `deepmd-kit/3.2.0 + abacus/LTSv3.10.1-sm70-auto`（Open MPI 5.0.8）模块链，否则 `prterun` exit 2；worker 级 sidecar 的 `mpi` 字段为 null 而 `counters_mpi.world_size` 有值（证据形状差异，已记小项）。

- 2026-09-22（**2.2.8 已发布**）：tag `v2.2.8` -> `b6a68f9f`；发布工作流 run `35711534156` 成功（同 commit 的 `Tests`/abacuslite 工作流亦通过）；PyPI 上线 wheel（395,804 B，sha256 `e686cce0…7718`）与 sdist（349,069 B，sha256 `e39e0d36…6108`）；隔离 venv 无缓存 clean-install 复核 `atst 2.2.8` / `package_version()==2.2.8`。release notes/README/矩阵状态改为已发布。此次一并回填 GPU 调优计划 4 行勾选（PT/TF 线程预算、`atst_runner.py` 核对、状态隔离复核、恒电势×runtime 组合回归）与 188 行的终审/发布口径，并给 convergence、abacuslite 两份计划加勾选说明（工作已交付、勾选未维护；剩余为消费方/暂缓项）。

- 2026-09-22（**2.2.8 候选收口与复验入库**）：SAI 作业 1448173（main `bc420aa`）复验通过——`DP_*` 线程键到达、MPS 三态探测为完成的否证、`self_reported` 620.139 MiB；证据入切片 `docs/reports/data/ATST_PT_MPS_CLOSEOUT_20260922/reverify/`。发版文档入口（README/USER_GUIDE_CN/CONFIG_REFERENCE/PYTHON_API_REFERENCE/DOCS_ARCHITECTURE/PYPI_RELEASE_AUTOMATION）已与 2.2.8 对齐，`tests/unit` 1195 passed / 0 failed 后推送。

- 2026-09-22（**2.2.8 候选准备**）：`pyproject` 2.2.7→2.2.8；新增 `docs/releases/RELEASE_NOTES_2.2.8.md`（Compatibility / Validation Matrix / 待发布 Publication Evidence 三段）；README badge 与状态行、`docs/index.md`、`CONFIG_REFERENCE`/`PYTHON_API_REFERENCE`/`FEATURE_STATUS_MATRIX` 头部、`DOCS_ARCHITECTURE` 均指向 2.2.8；patch 语义（无破坏性契约变更）。tag 与 PyPI 发布由维护者执行。
