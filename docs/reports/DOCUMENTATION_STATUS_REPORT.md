...
| `docs/superpowers/specs/2026-09-04-user-distribution-and-mpi-installation-design.html` | spec | 已确认并实施的 GitHub 规范源 / 维护者手动拉取的 Gitee 同内容只读镜像 / PyPI 分发、跨渠道用户入口、串行与 image-parallel MPI 安装分层，以及 SAI 维护证据边界设计。 |
| `docs/superpowers/plans/2026-09-04-user-distribution-and-mpi-installation.md` | plan | 已实施的用户分发、跨渠道 README 入口、串行与 image-parallel MPI 安装诊断、SAI 维护边界和文档事实修正 TDD 计划。 |
