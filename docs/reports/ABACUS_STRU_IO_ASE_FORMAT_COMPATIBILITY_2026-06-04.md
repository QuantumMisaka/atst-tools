...
这里的“本项目基本需求”指：NEB、AutoNEB、Dimer、Sella、CCQN、D2S、Relax、Vibration、IRC 等当前工作流需要读入 ABACUS 初末态 `STRU`，并通过 `atst abacus prepare` 写出可供 ABACUS 运行的 `INPUT`、`KPT`、`STRU`。这些路径主要依赖元素、cell、PBC、Direct/Cartesian 坐标、赝势、轨道基组、常规磁矩、ABACUS mobility / ASE constraints roundtrip，以及 `atst neb make` 中的 `--fix` / `--mag` 初猜链元数据设置；当前 abacuslite 路径可以覆盖这些需求。

但 `ase-abacus` fork 的 STRU I/O 功能边界更宽，包括 ASE I/O format 注册、Direct/scaled 坐标写出、真实初速度 roundtrip、verbose metadata readback 等。当前 ATST-Tools / abacuslite 尚未覆盖这些能力。
...
| Writer 参数 | `pp_file`、`orb_file`、`fname`；当前总是写 Cartesian 坐标，mobility 来自 ASE `FixAtoms` / `FixCartesian` constraints | `scaled`、`pp`、`basis`、`offsite_basis`、`init_vel`、`pp_basis_default` 等 | fork 功能更多 |
...
| 全冻结原子 mobility 读入 | `read_structure()` 将 `m 0 0 0` / `0 0 0` 转成 `FixAtoms` | 覆盖当前常见固定原子读入 | 覆盖 |
| 部分方向 mobility 读入 | `read_structure()` 将部分 `m` 值恢复为 `FixCartesian` | 覆盖 NEB STRU 端点固定方向输入 | 覆盖核心 ASE constraint 语义 |
| constraints 写回 mobility | `write_stru()` 将 ASE `FixAtoms` / `FixCartesian` 写回 ABACUS `m` flags | 覆盖 NEB image 和 ABACUS input 准备 | 覆盖核心 ASE constraint 语义 |
...
- **可以覆盖 `atst neb make` 的 STRU 输入需求**：ABACUS `STRU` / `.stru` 可作为初态、末态和 TS guess 输入；读入后生成的 ASE `Atoms` 保留 `m` mobility 约束，也可以继续应用 `--fix HEIGHT:DIR` 和 `--mag ELEMENT:MOMENT`。`linear` 和 `IDPP` 两种初猜链生成路径均已用 `examples/02_neb_H2-Au` 端点转换 STRU 后做一致性验证。
- **不能完整取代 `ase-abacus` fork**：需要完整 ASE I/O plugin 行为、Direct 坐标写出、真实 velocity roundtrip、verbose metadata readback 或扩展 basis/descriptor 块时，当前实现不够。
...
- `tests/unit/test_post_analysis_io.py` 中 June 5 后续测试：确认 Cartesian STRU 坐标、三分量磁矩可读，同时确认 `m 0 0 0` 保留为 `FixAtoms`、部分 mobility 保留为 `FixCartesian`，不会把 STRU velocity 转成真实 `Atoms` velocities。
- `tests/unit/test_abacuslite_profile.py::test_write_stru_preserves_first_occurrence_species_order`：确认 species 顺序按首次出现顺序。
- `tests/unit/test_abacuslite_profile.py` 中 June 5 后续测试：确认 `write_stru()` 写出 Cartesian 坐标、pp/orb 和磁矩，并将 ASE `FixAtoms` / `FixCartesian` 写回 ABACUS mobility；输入速度仍按当前策略写为零速度。
...
- `tests/unit/test_cli.py::test_neb_make_preserves_constraints_from_stru_mobility`：确认 `atst neb make` 从 ABACUS STRU 端点生成初猜链时保留全固定和部分方向 mobility constraints。
...
- 保留元素、cell、PBC、Direct/Cartesian 坐标、常规磁矩和 ABACUS mobility constraints；
...
原因是 `atst neb make` 现在对初态、末态和可选 TS guess 统一使用 `read_structure(..., parallel=False)`。当输入为 ABACUS `STRU` / `.stru` 时，该入口会通过 abacuslite parser 生成 ASE `Atoms`，并保留 `m` mobility 到 `FixAtoms` / `FixCartesian` constraints；随后 `set_fix_for_Atoms()` 和 `set_magmom_for_Atoms()` 仍可继续作用在中间 image 的 ASE `Atoms` 上。因此，`--fix HEIGHT:DIR` 和 `--mag ELEMENT:MOMENT` 不依赖 ASE fork 的 `abacus` I/O format 注册，也不依赖 `ase.io.read("*.stru")` 原生可用。IDPP 写回优化坐标时显式忽略输入 constraints 对坐标写回的影响，同时保留 constraints metadata 供 NEB chain 和后续 ABACUS image input 使用。
...
4. 没有真实初速度 roundtrip；
5. verbose metadata readback 与 fork 不一致；
6. 不覆盖 `ABFS_ORBITAL`、`NUMERICAL_DESCRIPTOR`、`latname` 等扩展能力。
