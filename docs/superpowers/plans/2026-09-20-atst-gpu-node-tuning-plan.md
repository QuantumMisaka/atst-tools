...
**版本**: 2026-09-20（2026-09-21 迁入 atst-tools 规范源；2026-09-22 状态口径对齐）
**日期**: 2026-09-20
**状态**: 执行中（P0–P5 已完成，P6 进行中；未闭合项见下方「剩余项（2026-09-22）」）
...
Status: in_progress (P0-P5 complete, P6 in progress; the remaining items are listed below)
...
分支级开放门与逐条证据索引另见[审阅地图 §4](../../reports/ATST_GPU_TUNING_BRANCH_REVIEW_MAP_2026-09-21.md)。
...
- [x] 宿主 sampler/parser 区分 device/process，覆盖缺工具、权限不足、短任务无样本、MPS 归属未知和采样失败（device 行与计算进程行、缺工具/权限/无进程原因、短任务零样本、采样失败均有实现与单测；**MPS 归属 2026-09-22 给出站点结论**：站点节点在跑 `nvidia-cuda-mps-control`/`nvidia-cuda-mps-server`，`nvidia-smi --query-compute-apps` 无稳定进程行，sampler 逐样本记 `unavailable`（reason `no compute process was reported`）而不填 0，可用时记 `observed`；2026-09-22 已实现**显式 MPS 探测**（进程表 + 客户端管道正证据，`detected`/`probes`/`evidence` 三态，空表时 reason 点名 MPS）与**自身归因** `self_reported`（本进程 torch 分配器高水位，不触发 CUDA 初始化，缺失记 `unavailable`+reason），见 `tests/unit/test_evidence_mps.py`；站点复验待下一轮作业，见 [P2 收口报告](../../reports/ATST_P2_CLOSEOUT_2026-09-22.md) §4.5）。
...
- [x] 真实运行前确定 fixture、各项数值容差、最大并发/时长/卡时与停止条件，实时核验 QOS 和实际 CPU/GPU 配额（2026-09-21 维护者“开跑”指令按本节拟值执行：FT²DP 单头 100k + 示例夹具；DP 并行等价 ≤1e-4 eV 量级；DP/ABACUS 作业分别 ≤1h/≤2h；QOS `rush-1o2gpu` 现场核验）。
- [x] 依据 SAI guide 核验提交方式、module/MPI、存储与 sampler；分别记录 ABACUS 环境和模型匹配 DP 环境，不能以版本下限代替 DPA4 加载验证（实测：deepmd-kit/3.2.0 链加载 FT²DP `ntypes=118` 通过；abacus/LTSv3.10.1 链与之共存；mpi4py↔OpenMPI 实测可初始化；Slurm 槽位规则与 `--oversubscribe` 记录在案；sampler 在站点产出 165/753 个样本）。
- [x] 先单 worker 身份/数值验证，再短矩阵；出现 OOM、数值不合格或并发收益饱和时停止扩大该分支（冒烟 1430846→1430966→1431010 逐层修复后通过；短矩阵 1431119 DP 12/12、1431200 ABACUS 4/4；无 OOM；并发收益在 V100 上持平，未扩大分支）。
...
- [x] 候选正式点至少三次交替重复，保留失败；host/SIF 成对验证，采样开/关检查观测开销（DP 已 3 次交替重复；ABACUS NEB 3 次（750.2/712.7/709.8 s）、relax 4 次（172.8/171.0/171.7/172.6 s）；全部失败样本保留；采样开/关 A/B 已有本地与站点两遍对照；**host/SIF 成对已完成**——2026-09-21 作业 1438496：同一 allocation 内 host 142 s vs SIF 153 s、末能量一致 −239256.2707 eV、两侧 sidecar complete，见 SAI 报告 §5j 与证据切片 `docs/reports/data/ATST_HOST_SIF_PAIR_20260921/`）。
- [x] 收尾补测（2026-09-21 维护者“继续推进”后执行）：修复后 ABACUS relax 基线（`omp: 1` 显式 vs manifest 预算 2：160.9 / 157.6 s，`threads_source=harness` 记入证据；该示例 4 rank 之上再加线程收益≈2%，不值得再堆）与 8 ranks/1 卡压力行，见 SAI 报告 §5d 与证据切片 `docs/reports/data/ATST_P5_CLOSEOUT_20260921/`。
...
- [x] NEB 图数 × 卡数映射（P0 复核新增）：4/8 内部图 × 1/2/4/8 卡；验证“图数 ≤ 卡数”的延迟收益与 8 ranks/1 卡的压力边界（站点已测：3 ranks/1 卡≈9.9 s、Σ35 次力调用；4 内部图 × 4 卡（`srun --mpi=pmix_v5 --gpus-per-task=1`）11.8/9.9 s、`world_size=4`、Σ38 次；**8 ranks/1 卡已于 2026-09-21 完成**（1436926/1436941：串行 11.2–13.6 s vs 8 rank 16.3–20.5 s，1.2–1.8×、显存峰值 16.6 GB / 32 GB，见 SAI 报告 §5d；本地 7.7× 的外推不适用于 V100）；**8 图/8 卡亦已完成**（1438379：`rush-gpu` 上限 16 卡，8 卡 24.31 s vs 串行 26.63 s——本 band 上仅 ≈1.1×，收益随每步工作量增长，见 SAI 报告 §5h）。该行关闭。
...
- MPI 站点行为（2026-09-21 实测）：`srun` 默认 `--mpi=none`（任务内无 PMI，mpi4py 退化为 size 1；多卡需 `srun --mpi=pmix_v5 --gpus-per-task=1`）；`mpiexec` 可正常初始化，但**带 runtime 请求的 runner 在 rank 内 `execve` 重绑定会挂起**（PMIx 会话失效；本地 MPICH 无此问题），因此站点多卡 NEB 用 srun+pmix 方案；`runtime.binding: round_robin` + `mpiexec` 组合在站点不可用（留档 1431646/1431952）；其根因含 runner 在 exec 前初始化 MPI 的顺序缺陷，已随第三轮审查修复（接口文档 §7.3）。作业槽位：分配按 `--ntasks` 计槽，`mpiexec -n N`/`mpirun -np N` 需 `--ntasks >= N` 或 `--oversubscribe`。
...
- 登台包（本地已备，授权后单向拷贝）：`~/scratch/atst-p5-staging-20260921/`——wheel、`git archive` 源码快照与 **`git bundle`**（124 MB；在 SAI 克隆为真实 git 检出，使 `bench_record` 的 `revision` 携带 `head/branch/dirty`；从 wheel/tarball 运行会得到 null revision）；三者文件名与 sha256 以该目录的 `MANIFEST.sha256` 为准（当前快照 = 提交 `9aa1ef9`），登台时按冻结 HEAD 重导出并刷新 MANIFEST；该套件本体已本机验证：wheel（无 `PYTHONPATH`）直跑产出含 `memory_peak_mib` 的 sidecar，bundle 克隆的 sweep+record 携带 `head=9aa1ef9`；权重 regular 单头 100k（`ft2dp-dpeva/scratch_atp_upload/model.ckpt-100000.pt`，62 MB，sha256 `13b74797…`；`mission_20260920/FT2DPv2.2-single100k-model.ckpt-100000.pt` 为同一文件）；EMA 单头 100k（pin `45667e7f…`）本机仍无（pin 记分卡记 EMA ≈ regular，故默认 regular-only、EMA 复核后补；本机另有 multi200k regular+EMA 包，可提请替代）；通用回归模型 `temp_repos/dp_model/DPA-3.1-3M.pt` 已在本地（sha256 `86dd3a80…`）。 P5 清单草案 `cases.p5.draft.json`（双后端 6 个 case，`configs/` 4 份配置齐备并经 schema 校验，每 case 独立 workdir）随登台包一并拷贝；清单已用 `load_cases` 预解析通过。
...
**P5 开跑前待裁决（拟值，可直接批注）——2026-09-21 维护者回复“开跑”，以下拟值即执行依据**：
...
harness 语义：case 默认在配置文件所在目录运行（ATST 相对路径按 cwd 解析），显式 `workdir` 则相对批量输出目录解析；报告/日志/`atst_api_result.json` 一律在 `<out>/<case_id>/`。**同一 manifest 内两个 case 不得共享非空 `workdir`**（加载即拒绝：并发或重复运行会互相覆盖 sidecar/traj/结果）；P5 清单为每个 case 使用独立相对 workdir（如 `work-<case_id>`）。站点资源现场事实见上方 SAI 只读勘察块。
