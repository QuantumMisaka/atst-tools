...
- [x] 宿主 sampler/parser 区分 device/process，覆盖缺工具、权限不足、短任务无样本、MPS 归属未知和采样失败（device 行与计算进程行、缺工具/权限/无进程原因、短任务零样本、采样失败均有实现与单测；**MPS 归属 2026-09-22 给出站点结论**：站点节点在跑 `nvidia-cuda-mps-control`/`nvidia-cuda-mps-server`，`nvidia-smi --query-compute-apps` 无稳定进程行，sampler 逐样本记 `unavailable`（reason `no compute process was reported`）而不填 0，可用时记 `observed`；2026-09-22 已实现**显式 MPS 探测**（进程表 + 客户端管道正证据，`detected`/`probes`/`evidence` 三态，空表时 reason 点名 MPS）与**自身归因** `self_reported`（本进程 torch 分配器高水位，不触发 CUDA 初始化，缺失记 `unavailable`+reason），见 `tests/unit/test_evidence_mps.py`；站点复验见报告 §9（仅 MPS 正向路径仍只有单测覆盖），见 [P2 收口报告](../../reports/ATST_P2_CLOSEOUT_2026-09-22.md) §4.5）。
