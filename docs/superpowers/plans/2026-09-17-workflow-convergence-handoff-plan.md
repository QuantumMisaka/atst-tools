...
- A4 review (`588f2b2..c3df747`, independent read-only child): 1 Critical / 2 Important / 3 Minor. Clean: sella/relax ownership + API preservation (no stale reuse), single-owner audit across every constituent construction site, synthesized stage shape, language gate, no extra calculator calls. Critical: the ten fixture JSONs were silently excluded by the repository `.gitignore` `*.json` rule and never committed, so a clean checkout failed the new test. Important: `vibration`/`md` stage records omitted `converged` (contradicting the documented always-present tri-state contract), and two fixtures encoded producer-impossible states with no committed generator.
- A4 review repairs (`2bd8e8f`): `.gitignore` negation + explicitly pinned fixture set (CI fails loudly if a fixture is missing); committed `scripts/generate_convergence_fixtures.py` and regenerated every fixture (synthesized fixture is now a real sella shape; D2S vibration is complete alongside its artifacts); `vibration`/`md` records now serialize `converged: null` through `StageRecord` while keeping md's extra keys; standalone-Dimer doc bullet corrected; sella/relax docstrings describe `artifact_manifest=None` as an internal programmatic contract; the NEB rank test now compares full stage dicts across ranks. Full suite 824 passed, 19 skipped; documentation governance passed.
