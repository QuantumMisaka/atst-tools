...
当前 2.2.4 版本（候选，待发布；是否正式发布以 exact tag 和 PyPI 包为准，详见 [release notes](../releases/RELEASE_NOTES_2.2.4.md)）把原 main branch 的脚本集合整理为可安装 Python package，
...
复用同一 schema、获得结构化结果，或向 ATST 的 CCQN 传入已创建的 ASE
calculator 时，使用稳定的 `atst_tools.api`。两条路径复用校验和 workflow
服务；Python API 不启动调度器或 MPI，也不会重建传入 CCQN 的 calculator。详见
...
`2` 为 ATST API 错误，`1` 为意外 runner 错误。调度和外层 MPI 由调用方负责。
...
- 使用 GPU 的 ABACUS LCAO 计算，应依据已安装 ABACUS 的文档选择与站点兼容的
  GPU solver。
...
`summary` 子命令只读已有轨迹、cache 或阶段输出，可用于调度任务监控和最终结果摘要；
...
- 参数总表：[YAML_INPUT_VARIABLES.md](YAML_INPUT_VARIABLES.md)。该文件按已安装版本的
  schema 生成，是非 calculator YAML 字段的主入口。
- 快速检查：
...
开发新 workflow、配置字段、示例或发布时，请使用
[维护交接清单](../developer/HANDOVER.md) 和
[示例验证操作指南](../developer/EXAMPLE_VALIDATION_OPERATIONS.md)。
